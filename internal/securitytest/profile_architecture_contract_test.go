...
// This file implements the SYNTACTIC half of the Phase 2 plan's
// "Mechanical diagnostic enforcement" (L887-898), "Mechanical architecture
// contract" (L1311-1337), Amendment 3 (the closed `reason` vocabulary must
// be enforced the same way as `diagnostic`), and Amendment 4 (this untagged
...
// with go/parser — go/ast only, which is the right tool here because every
// invariant this file keeps is a property a parse tree states outright —
// and asserts, of the plan's thirteen structural invariants, the genuinely
// syntactic ones (the numbering is kept sparse so cross-references between
// the two contract files stay stable):
//
...
//
// The other four invariants — 4 (the only map-typed state is Server's
// active-connection map), 6 (the only channels are serveDone/stopDone),
// 12 and 13 (Verifier.Verify / RoleResolver.Roles referenced exactly once
// each, in bind.go) — are TYPE-SEMANTIC: they quantify over every value of
// a given type, however that value is produced, which no go/ast shape
// enumeration can decide. This file used to carry an ever-growing shape
// enumerator for them; three consecutive external review passes each found
// a new ordinary-Go bypass (make() forms, then composite literals and
// method values, then var-composite-literals/function result types/
// transitive aliases), and a final architecture consultation produced
// still-open counterexamples (maps.Clone of a map, time.After(), a
// CallExpr receiver, generic instantiation at a map type) that closed the
// question: those four invariants are now enforced type-aware, over the
// fully type-checked package, in profile_types_contract_test.go — see that
// file's header for exactly what the go/types-based checks prove and their
// stated residual limits.
//
// Sabotage checks for THIS file's invariants (run manually, restored
// afterward): `diagnostic(err.Error())` at a result callsite; a second
// `go` statement; a blank goldap import; making decodeMembershipFilter
// recurse; a `reason(fmt.Sprint(err))` conversion. Map/channel and
// Verify/Roles sabotage evidence lives with the type-aware checker in
// profile_types_contract_test.go.
//
...
type profileFile struct {
	name string // base filename, e.g. "server.go" — used in failure messages
	fset *token.FileSet
	file *ast.File
}
