...
- **Issue #33 phase 3's tagged replacement-composition gate.** `cmd/ch-oauth-ldap`
  ...
  AND `TestDependencyContract_Phase3ReplacementCommandTests`, which performs a
  real, deterministic `go test -mod=readonly -tags=phase3profile
  ./cmd/ch-oauth-ldap` — so CI both compiles AND executes the tagged
  composition's own tests (`config_phase3profile_test.go`,
  `main_phase3profile_test.go`), even though the five required workflow
  commands themselves stay untagged and unchanged; CommandBuilds alone would
  prove only that the tagged composition compiles, not that its tagged test
  files' assertions actually pass. Ordinary untagged builds, and the
  published `Dockerfile.ch-oauth-ldap` production image, always select the
  legacy `internal/ldap` adapter regardless of any of the above.
