...
- `ch-oauth-ldap`: a standalone LDAPv3 server where the simple-Bind password
  ...
  ClickHouse (24.8 baseline, 25.8, 26.3 and 26.8) by the
...
| `.github/workflows/build-ch-oauth-ldap.yml` | push-to-main image publication for `ghcr.io/altinity/ch-oauth-ldap` (tag `ldap-<short-sha>`), mirroring `build-ch-jwt-verify.yml`'s structure. Also repoints a moving `:latest` alias at that immutable manifest — a pointer created with `imagetools create`, never a build, and only on a default-prefix push to `main`. The immutable tag stays the artifact of record and the only thing the no-force republication guard protects; `helm/ch-oauth-ldap` still expects a pinned tag in its values, and `helm/ch-oauth-ldap/ci/lib/image-assertions.sh` enforces the alias's pointer-only shape (plan §21, amended) rather than banning the `:latest` string outright |
...
- `./integration/clickhouse/run.sh` (or `run-all-builds.sh`) — the real-ClickHouse
  ...
  The two original tracked images (`24.8.11.51285.altinitystable`,
  ...
  see `docs/clickhouse-ldap-wire-profile.md` §11.5/§11.6. The 26.3
  (`altinity/clickhouse-server:26.3.16.10001.altinitystable`) and 26.8
  (`clickhouse/clickhouse-server:26.8.1.2041`, upstream — no Altinity
  Stable 26.8 build exists) lines were added later and certified the same
  way; see §11.7. All four are in `run-all-builds.sh`'s `BUILDS`.
  ...
  at `--mode verify` **for every already-committed tracked line** — do not
  regenerate or promote any committed
  `internal/ldap/testdata/clickhouse-wire/**` fixture; a verify pass is
  production-backed verification of the historical corpus, not new fixture
  provenance, and a new request shape from a tracked client stops the unit
  rather than broadening the parser. `--mode generate` remains available
  for a NEW tracked line that has no committed fixture directory yet (the
  26.3/26.8 addition used it); the script itself refuses to overwrite a
  committed line's directory, so the narrowed policy is mechanical rather
  than a convention to remember (see `integration/clickhouse/README.md`'s
  wire-capture note).
