...
for (const [waveIndex, wave] of waves.entries()) {
  ...
  const integration = await agent(
    ...
    `If every merge succeeds cleanly, run the full local gate (` + '`go build ./... && go vet ./... && go test ./...`' + `, captured to a file, tail only) and report gateGreen/gateTail truthfully — a clean set of individually-gated branches can still fail to gate together.\n\n` +
