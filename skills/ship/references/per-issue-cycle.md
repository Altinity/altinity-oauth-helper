...
**Medium:** cooperating modules, config/wire-format changes, non-trivial impact on
`examples/` consumer wiring or the Helm chart, or shared type changes.
...
- Keep JWKS/JWT/identity-policy logic in the upstream `go-mcp-oauth-sdk` module, not
  ...
  the `/verify` handler, caching, settings) stays local.
- Update every real fixture affected by a config/handler shape change (e.g. the
  in-process `testIdP` in `cmd/ch-jwt-verify/verify_test.go`). Do not preserve
...
There is no `pretest`/coverage gate wired into tooling here — run the gate explicitly,
captured to a file, and loop until green:

```sh
{ go build ./... && go vet ./... && go test ./...; } > "$TMPDIR/gate.log" 2>&1
```

Never proceed on a red suite or a broken build. There is no automated e2e/browser
suite in this repo — for a change visible to a consumer (wire contract, Helm chart, an
`examples/` recipe), sanity-check it against the relevant example (e.g.
`examples/curl/verify.sh`, or by bringing up `examples/_platform`'s docker-compose
stack) before calling the unit done.
...
10. The full local gate is green (`go build ./... && go vet ./... && go test ./...`);
    a consumer-visible change was sanity-checked against the relevant `examples/`
    recipe.
...
- Update `CHANGELOG.md` under `[Unreleased]` when required (the worker writes its own
  ...
  entry — there's no other unit's entry sharing this PR to dedupe against). This repo
  does not currently keep a `CHANGELOG.md` — skip this bullet unless one exists by the
  time you ship.
- Update the relevant ADR addendum for architecture changes, if this repo has adopted
  ADRs by the time you ship (it has none as of this writing).
...
- Out-of-scope bug or footgun spotted → a **separate** issue (label it `inbox` if that
  label exists in this repo; otherwise say plainly in the title/body that it's
  out-of-scope and why deferred, with file:line); never fold it into this PR.
