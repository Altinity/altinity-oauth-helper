...
of live check unprompted (e.g. confirming an exact Go module version) — so this loop
...
`grep`/`find` in Bash are intercepted in this sandboxed environment — use `rg PATTERN >
"$TMPDIR/out"` then Read (and `command find` if you need real `find`). Capture long
command output to files (`go test ./... > "$TMPDIR/test.log" 2>&1`); Read only the tail
on green and the focused failure sections on red — `go test -v` output across many
packages is the main accidental context sink in a run.
...
A prior unit's own recorded outcome (a decision logged in the ship log, or an ADR if
this repo adds one later) is context worth reporting alongside a gate — it explains *why*
...
- Go module dependencies resolve automatically from `go.sum` on `go build`/`go test` —
  there is no separate install step. Run `go mod tidy` only when a unit intentionally
  changes dependencies, and diff `go.mod`/`go.sum` before committing.
...
2. **Reconcile per cycle step 4 — this unit's own entry only.** One unit per PR means
   ...
   keep one — skip this bullet unless one exists by the time you ship), close or
   reconcile any issue this unit itself supersedes, and tick only this unit's own
   `## Phases` row (if the issue carries that checklist). All of this lands **now,
   before the PR** — nothing may be pushed after certification (2.6).
3. **Open this unit's PR** (`gh pr create --base main`), title `<type>(#<ISSUE>):
   ...
   per `.github/PULL_REQUEST_TEMPLATE.md` if one exists (this repo has none as of this
   writing — use its structure as your own outline instead) covering this unit's
   contract coverage, invariant verification, sabotage cases, tests, and build results;
   `Closes #<n>` only if this unit completes the whole issue, `Part of #<n>` otherwise;
   the repo PR footer.
