...
// sizeInt64 and writeInt64 are the ORIGINAL, unmodified upstream
// implementation. They stay unsigned-magnitude (grow only until the value
// shifts down to zero, no sign-bit check) deliberately: writeTagAndLength
// below calls writeInt64 directly to encode a BER length octet run, which is
// always a plain non-negative magnitude with no two's-complement sign to
// disambiguate — patching this pair the way sizeInt32/writeInt32 needed
// patching would instead break every length >= 128 (see ../PATCHES.md item
// 1, which explains why the fix lives on the INTEGER-specific pair below
// instead of here).
...
// sizeInt32 and writeInt32 are patched from upstream goldap: see
// ../PATCHES.md item 1. Unlike sizeInt64/writeInt64 above, these two are
// used exclusively to encode an actual ASN.1 INTEGER/ENUMERATED value
// (bytes.go's WritePrimitiveSubBytes/SizePrimitiveSubBytes, reached from
// integer.go's INTEGER.write for every MessageID) — a genuinely signed,
// two's-complement quantity, not a length. Upstream reused sizeInt64/
// writeInt64 here too, so it inherited the same missing sign-disambiguation
// padding: a value needing it (e.g. 128, whose minimal correct DER encoding
// is two bytes 0x00 0x80, not one byte 0x80 — which a BER/DER reader decodes
// back as -128) was mis-encoded. message.MessageID is exactly this INTEGER
// type, and go-ldap/v3 (and every real LDAP client) increments a
// connection's message ID by 1 per request starting at 1, so any connection
// issuing 128+ requests hit this every time, silently desynchronizing
// response correlation from that point on.
//
// This is the standard minimal two's-complement algorithm (compare
// github.com/go-asn1-ber/asn1-ber's int64Length/marshalInt64): grow the byte
// count while the remaining value doesn't fit in one signed byte, then emit
// that many bytes least-significant-byte first (writeBytes fills the shared
// buffer back-to-front, so byte order here ends up big-endian in the final
// wire encoding — same convention sizeInt64/writeInt64 above rely on).
func sizeInt32(i int32) (size int) {
	size = 1
	for i > 127 {
		size++
		i >>= 8
...
func writeInt32(bytes *Bytes, i int32) (size int) {
	size = sizeInt32(i)
	for n := 0; n < size; n++ {
		bytes.writeBytes([]byte{byte(i)})
		i >>= 8
