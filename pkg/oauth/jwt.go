...
// parseAndVerifyExternalJWT parses a compact-serialised JWT, fetches the
// JWKS for the configured issuer (with a one-shot kid-rotation refresh),
// and returns the validated claims. Issuer enforcement (singular
// config.Issuer) and audience enforcement (expectedAudience) both happen
// here, slash-normalised.
