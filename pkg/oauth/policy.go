...
// rather than configurable so every Verifier in a deployment shares the same
// window.
const clockSkewSecs = int64(60)

// EmailDomain returns the lowercased domain portion of an email address, or
// "" when the input is malformed. Trimmed first so leading/trailing
// whitespace doesn't smuggle past the @ split.
...
// and whitespace-insensitively. Used by callers for allowed_email_domains /
// allowed_hosted_domains policies.
