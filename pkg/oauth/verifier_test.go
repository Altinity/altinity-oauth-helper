...
// encodeOIDC returns a JSON-encodable discovery document with the standard
// fields used by the discovery helper.
...
func TestResolveJWKSURL(t *testing.T) {
	...
	t.Run("oauth_authorization_server_discovery", func(t *testing.T) {
		t.Parallel()
		// Discovery tries oauth-authorization-server first, falling through
		// to openid-configuration on 404. Same shape works for both — pick one.
		var mockURL string
		mux := http.NewServeMux()
	...
	t.Run("missing_jwks_uri_in_metadata", func(t *testing.T) {
		t.Parallel()
		// Emit a document with a matching issuer but no jwks_uri to exercise
		// the Verifier's "missing jwks_uri" branch.
		var mockURL string
		mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "application/json")
...
// back to the package default (5 minutes).
...
// triggers a one-shot cache-bypass re-fetch.
